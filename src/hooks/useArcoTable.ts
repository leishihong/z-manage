...
export const useArcoTable = ({ fetchApi, state, fetchStatusApi, fetchDeleteApi }: IProps) => {
	...
	const [initialValues, setInitialValues] = useState<{ [key: string]: any }>(state);
	...
	const handleSearch = useCallback(
		(value: any) => {
			...
			setInitialValues((preState: any) => Object.assign({}, preState, { ...value, startTime, endTime }));
			queryTableList({
				currPage: 1,
				pageSize: pagination.pageSize,
				endTime,
	...
	const handlePageChange = useCallback(
		({ current, pageSize }: PaginationProps) => {
			setPatination((preState) => Object.assign({}, preState, { current, pageSize }));
			queryTableList({
				currPage: current,
				pageSize,
				...initialValues,
		...
		async (record: any, actionType: string, pathname?: string) => {
			...
			switch (actionType) {
				...
				case 'edit':
					...
					break;
				case 'status':
					modalConfirm({ content: `确定${statusText}吗？` }, async (callback) => {
						try {
							const params = { status: record.status ? 0 : 1, id: record.id };
							const { status } = await fetchStatusApi(params);
							if (status === 200) {
								const findInd = tableList.findIndex((item) => item.id === record.id);
								if (findInd > -1) {
					...
					break;
				case 'delete':
					modalConfirm({ content: '删除后不可恢复，确定删除？' }, async (callback) => {
						try {
