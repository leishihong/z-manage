...
const PDFPreview: FC<IProps> = (props) => {
	const { src, getContainer = document.body, withCredentials = true, initialPage = 0 } = props;
	...
	const renderError = (error: any) => {
		...
		return (
			<div className={cx['pdf-viewer-load-error']}>
				<Result status="error" title="PDF加载失败，请稍后重试" subTitle={`造成失败的原因如下：${message}`} />
			</div>
		);
	};
	//TODO: 改写pdf默认加载器
	const renderLoading = (percentages: any) => {
		...
		return <Spin spinning={true} tip="pdf正在加载中，请耐心等待..." size="large" />;
	};
