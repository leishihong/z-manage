import { useEffect, useRef, useState, useCallback, FC, memo, useMemo } from 'react';
import { Modal, Form, Input, Checkbox, Button, Space, Spin, Tabs, Row, Col } from 'antd';
...
import { setLoginInfo, setAuthToken, setRememberPwd, ILoginState } from 'store/loginSlice';
...
const Login: FC = () => {
	...
	const { loginInfo, rememberPwd }: ILoginState = useAppSelector(({ loginState }) => loginState);
	const dispatch = useAppDispatch();
	const [rememberPassword, setRememberPassword] = useState<boolean>(rememberPwd);
					...
					<Spin spinning={loading}>
						<div className={cx['login-form-wrapper']}>
							...
							<Form form={form} className={cx['login-form']} layout="vertical">
								{loginType === 'account' && (
									<Form.Item noStyle>
										<Form.Item name="userCode" rules={[{ required: true, message: '账号不能为空' }]}>
										...
										<Form.Item noStyle>
											<div className={cx['captcha-code-wrapper']}>
												<Form.Item name="verificationCode" rules={[{ required: true, message: '验证码不能为空' }]}>
													<Input allowClear placeholder="请输入验证码" onPressEnter={onSubmitClick} />
												</Form.Item>
												<CaptchaCode
								...
								<Button className={cx['login-form-submit-btn']} type="primary" shape="round" block onClick={onSubmitClick}>
									登录
								</Button>
								<Form.Item>
									<Space align="center" className={cx['login-form-password-actions']}>
										<Form.Item name="rememberMe" valuePropName="checked" noStyle className={cx['login-form-remember']}>
											<Checkbox
