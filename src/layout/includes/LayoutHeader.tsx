...
import { Tooltip, Input, Avatar, Modal, Dropdown, Layout, Breadcrumb, Button } from 'antd';
...
const Navbar: FC = () => {
	const { settings, userInfo, userLoading, isCollapsed, breadcrumbList } = useAppSelector(({ globalState }) => globalState);
				...
				{settings.breadcrumb && (
					<Breadcrumb>
						...
						{map(breadcrumbList[pathname], (item: string) => {
							return item !== '首页' && <Breadcrumb.Item key={item}>{item}</Breadcrumb.Item>;
						})}
					</Breadcrumb>
				)}
			</div>
			<ul className="right">
				<li>
					<Tooltip title={isFullscreen ? '退出全屏' : '全屏'}>
						<IconButton onClick={toggleFullscreen} icon={isFullscreen ? <FullscreenOutlined /> : <CompressOutlined />} />
					</Tooltip>
				</li>
				{userInfo && (
					<li>
						<Dropdown menu={{ items: menuItemList, onClick: handleMenuClick }} placement="bottomRight" disabled={userLoading}>
							<Avatar size={36} style={{ cursor: 'pointer' }} src={<img alt="avatar" src={AvatarIcon} />} />
						</Dropdown>
					</li>
				)}
			</ul>
